...
class EnvironmentManager:
    ...
    def _save_environments(self, environments: List[Environment]) -> None:
        ...
        try:
            ...
            logger.info("Saving %d environments to file %s", len(environments), self.db_file)
    ...
    def load_environments(self, folder_id: Optional[str] = None) -> List[Environment]:
        ...
        for env in environments:
            ...
            try:
                ...
                env.status = "dead"
                logger.warning("Container for environment %s not found, setting status to 'dead'", env.id)
            except Exception as e:
                logger.error("Error updating container status for environment %s: %s", env.id, e)
        ...
        if folder_id:
            ...
            logger.debug("Returning %d environments after filtering", len(filtered_envs))
    ...
    def _get_device_requests(self, runtime: str) -> Optional[List[DeviceRequest]]:
        device_requests = [DeviceRequest(count=-1, capabilities=[["gpu"]])] if runtime == "nvidia" else None
    ...
    def _create_container_config(self, env: Environment) -> tuple:
        ...
        logger.debug(
            ...
            env.id, mounts, port, command, device_requests
        )
    ...
    def duplicate_environment(self, env_id: str, new_env: Environment) -> Environment:
        logger.info("Duplicating environment with original id: %s to new environment: %s", env_id, new_env.name)
        environments = self.load_environments()
        original = self._find_environment(env_id, environments)

        if original.status == "created":
            logger.error("Cannot duplicate environment %s because its status is 'created'", env_id)
        ...
        logger.info("Creating container for duplicated environment with name: %s", new_env.container_name)
    ...
    def update_environment(self, env_id: str, update: EnvironmentUpdate) -> Environment:
        ...
        if update.folderIds is not None:
            logger.debug("Updating folderIds for environment %s to %s", env_id, update.folderIds)
    ...
    def _stop_other_environments(self, current_env_id: str, environments: List[Environment]) -> None:
        logger.info("Stopping other running environments excluding id: %s", current_env_id)
        for env in environments:
            if env.id != current_env_id and env.status == "running":
                ...
                try:
                    ...
                    logger.warning("Container for environment %s not found during stop operation", env.id)
                    continue
    ...
    def activate_environment(self, env_id: str, allow_multiple: bool = False) -> Environment:
        ...
        if env.status == "created":
            ...
            comfyui_path = Path(env.comfyui_path)
            mount_config = env.options.get("mount_config", {})
            custom_nodes_installed = self.docker_iface.copy_directories_to_container(container.id, comfyui_path, mount_config)
    ...
    def _hard_delete_environment(self, env: Environment, environments: List[Environment]) -> None:
        ...
        try:
            container = self.docker_iface.get_container(env.id)
            logger.debug("Stopping container for environment %s with timeout %s", env.id, SIGNAL_TIMEOUT)
            self.docker_iface.stop_container(container, timeout=SIGNAL_TIMEOUT)
            ...
            logger.warning("Container for environment %s not found during hard delete", env.id)
    ...
    def _prune_deleted_environments(self, environments: List[Environment], max_deleted: int) -> None:
        ...
        logger.info("Pruning deleted environments: found %d, max allowed %d", len(deleted_envs), max_deleted)
    ...
    def delete_environment(self, env_id: str, max_deleted: int = 10) -> str:
        ...
        if DELETED_FOLDER_ID in env.folderIds:
            logger.info("Environment %s marked as deleted, proceeding with hard delete", env.id)
            self._hard_delete_environment(env, environments)
        else:
